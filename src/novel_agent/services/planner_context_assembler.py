...
from novel_agent.domain.memory import RetrievalUnit, Stage1ContextPackage
...
class PlannerContextAssembler:
    ...
    def assemble(
        ...
        *,
        ...
        if stage1_context is not None:
            anchor_handles = {
                unit.compact_handle or unit.unit_id
                for unit in self._context_units(stage1_context, include_raw=False)
            }
            for unit in self._context_units(stage1_context, include_raw=False):
                item, receipt_ref = self._unit_item(unit)
            ...
            for unit in stage1_context.raw_evidence_spans:
                if unit.expanded_from_handle is not None:
                    expanded_by_handle[unit.expanded_from_handle].append(unit)
            for handle in sorted(anchor_handles, key=lambda item: item.root):
                ...
                for unit in expanded:
                    item, graph_ref = self._unit_item(unit)
                    optional.append(item)
    ...
    def _unit_item(self, unit: RetrievalUnit) -> tuple[PlannerContextItem, ArtifactRef | None]:
        section = self._section(unit)
        graph_ref = None
        if unit.graph_path_receipt is not None:
            graph_ref = self._artifacts.put(
                canonical_json_bytes(unit.graph_path_receipt.model_dump(mode="json")),
                "application/vnd.novel-agent.typed-graph-path+json",
                self._schema_version,
            )
        return (
            PlannerContextItem(
                ...
                graph_path_receipt_refs=() if graph_ref is None else (graph_ref,),
                compact_handle=unit.compact_handle,
            ),
            graph_ref,
        )
    ...
    @staticmethod
    def _section(unit: RetrievalUnit) -> PlannerContextSection:

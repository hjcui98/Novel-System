...
PLANNING_TURN_OUTPUT_CONSTRAINTS = (
    ...
    "HISTORY_RETRIEVAL_CONSTRAINTS=For every chapter_goal with chapter_index >= 2, "
    "history_retrieval.requirement MUST strictly be 'REQUIRED' (never 'NOT_REQUIRED'; the "
    "model cannot issue waivers). Provide 1 to 3 needs with kinds chosen from "
    "[causal_history, knowledge_origin, relationship_origin, setup_evidence, object_origin]. "
    "Each need MUST specify source_chapter_end strictly less than chapter_index "
    "(e.g. for chapter 6, source_chapter_end must be <= 5). Each need query must strictly "
    ...
    "ARC_VOLUME_ROADMAP_CONSTRAINTS=Every volume item must also carry chapter_set_roadmap: an "
    "ordered list of coarse plot segments covering exactly that volume's own chapter range, "
    "each with slot_id, chapter_start, chapter_end, plot_summary, key_cast, element_refs and "
    "expected_turn. Consecutive segments must be adjacent (previous chapter_end + 1 equals the "
    "next chapter_start) with no gap, no overlap and no chapter outside the volume, and different "
    "segments must not repeat one summary. The roadmap is future intent: it is never a refined "
    "chapter set, never an accepted window, and never evidence that anything already happened. "

...
gate_summary() {
  ...
  "${PYTHON}" - "${OUTPUT}" "${checkpoint}" "${expected_total}" <<'PY'
import json
import sys
...
if [[ ! -f "${OUTPUT}/progress_manifest.json" ]]; then
  run_stage genesis --stop-after-genesis
fi

last_chapter="$(${PYTHON} -c 'import json,sys; print(json.load(open(sys.argv[1], encoding="utf-8"))["last_accepted_chapter"])' "${OUTPUT}/progress_manifest.json")"

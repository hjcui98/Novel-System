...
进入 G0 需要先满足的条件，逐条核对如下：

| 条件 | 状态 |
|---|---|
| N0—N4 修复落地并有定向证据 | **完成**（提交 `fc25330`…`90f1533`） |
| D0 收敛与物化预检 | **完成**，含真实 Planner 修订与真实复审（`a1e8e1f`） |
| 残留 `RUNNING` 处置 | **完成**：全库 `unsettled attempts = 0`、`status='running' = 0`（`840865a`） |
| 驱动可定位且退出码真实 | **完成**（`scripts/60_run_stage.sh`，SHA 随仓库） |
| 说明 v23 为何不能继续作为 G0 载体 | **完成**（`v23_poison_loop_forensics_20260913.md`） |
| 新 project/run 身份 | **未决定** —— 指导第 12.1 节第 4 条明确"选择未占用身份，不预设一定是 v24"，这是需要操作者确认的选择 |
| 冻结配置（源码 SHA、prompt/skill 指纹、policy、profile、真实模型身份、检索配置） | **未记录**：v23 的 `environment.sh` 记录了 profile 与检索配置，但 G0 需要在新身份上重新冻结并留档 |

### 为什么 G0 不能复用 v20—v23

四条运行都停在**同一个阶段** ARC_VOLUME，且 v21/v23 的规范分类是 `poison_loop`。
v23 的现场证据（第 3 节）显示其失败形态是"无结构化意见的 `REVISE` 驱动整份重写"——
正是 N1/N3 修复的对象。因此：

- 新代码在旧身份上的行为不可比（指导第 12.1 节：不在长跑中改 prompt，也不对新代码 advance 旧冻结身份）。
- 旧运行的 basis 已被 STORY 提交固定，其候选与审校都是修复前的产物。
- 复用旧身份会把"修复后能否收敛"这一唯一待验证的问题与旧状态混在一起。

### 启动 G0 前的最后一个建议动作

本轮已用**真实模型**证明"一次限定修订 + 复审"能在修复后的通路上收敛
（`d0_delivery_20260913.md` 第 4.1 节）。仍缺的是**真实模型越界时恢复路径被真实触发**。
这不阻塞 G0（确定性用例已覆盖该路径），但若要在 G0 前消除最后一项不确定性，
可用同一诊断把一个越界修订喂给 `compose_scoped_revision` 并断言恢复 + 复审仍 ACCEPT。

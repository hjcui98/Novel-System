...
def test_real_qwen38_8006_profile_is_explicit_and_contract_bounded() -> None:
    endpoints = resolve_registered_model_endpoints(QWEN38_27B_NVFP4_8006_ENDPOINT_PROFILE)

    assert len(endpoints) == 1
    registration = endpoints[0]
    adapter = registration.adapter
    assert isinstance(adapter, OpenAICompatibleChatEndpoint)
    assert registration.endpoint_name == "qwen38-27b-nvfp4@8006"
    assert registration.model_name == "qwen38-27b-nvfp4"
    assert registration.revision == "qwen38-27b-nvfp4"
    assert adapter.base_url == "http://127.0.0.1:8006/v1"

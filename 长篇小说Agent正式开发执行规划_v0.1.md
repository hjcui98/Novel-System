...
12. **门禁不因简化而降低**：最小实现仍必须完成严格类型、权限/泄漏边界、失败与恢复语义、必要观测、迁移、回归测试、可复现实验和本阶段 Gate；削弱这些保证不是简化，而是未完成。
...
6. 文档细化同样遵守最小充分工程：优先更新现有权威文档，只在当前层级无法表达重大新决定时新增文档；不得为同一任务建立平行规划、治理或状态体系。
